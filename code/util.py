...
def print_sentence(output, sentence, labels, predictions, probs=None):
    spacings = [max(len(sentence[i]), len(labels[i]), len(predictions[i])) for i in range(len(sentence))]
    if probs is not None:
        spacings = [max(spacings[i], len("{:.2f}".format(probs[i]))) for i in range(len(sentence))]
    ...
    if probs is not None:
        output.write("p : ")
        for token, spacing in zip(probs, spacings):
            token_str = "{:.2f}".format(token)
            output.write(token_str)
            output.write(" " * (spacing - len(token_str) + 1))
        output.write("\n")

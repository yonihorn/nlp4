...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
        ...
        # add start_tokens to the beginning and end_tokens to the end
        _sentence = window_size * [start] + sentence + window_size * [end]
        _labels = window_size * [""] + labels + window_size * [""]
        _sentence_beginning = window_size
        _sentence_end = len(_sentence) - window_size
        for i in range(_sentence_beginning, _sentence_end):
            window = sum(_sentence[i-window_size:i+window_size+1], [])
            label = _labels[i]
...
class WindowModel(NERModel):
    ...
    def create_feed_dict(self, inputs_batch, labels_batch=None, dropout=1):
        ...
        if labels_batch is not None: # 'is not None' must be added because it's an array
    ...
    def add_prediction_op(self):
        ...
        W = tf.get_variable('W', shape=[self.config.embed_size * self.config.n_window_features, self.config.hidden_size], initializer=tf.contrib.layers.xavier_initializer())
    ...
    def add_loss_op(self, pred):
        ...
        _loss = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=pred, labels=self.labels_placeholder)
...
def do_train(args):
    ...
    embeddings = load_embeddings(args, helper)
    config.embed_size = embeddings.shape[1]
    logger.debug(config.output_path)
    #pdb.set_trace()
